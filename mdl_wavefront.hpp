...
{             // how to put all into.h file --> s. Vec3f.hxx    
  ...
  public:
    ...
    uint32_t          idGLTexture;
    std::string       s_Material;
    CMaterial       * p_Mat;
    // read the model -->
    std::vector<vec3> vertices;
    std::vector<vec2> uvs;
    std::vector<vec3> normals; // won't be used at the moment. (for bumpmaps??)
  };
  ...
  public:
    ...
    vec3        Kd;     //  diffuse reflectivity (rgb) <-- use this in the first place
    vec3        Ks;     // specular reflectivity (rgb)
    vec3        Ke;     // ? (not in Paul Bourke's description)
